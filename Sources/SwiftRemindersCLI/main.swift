...
struct ObsidianTask {
    let id: String  // UUID for the task
    let text: String
    let dueDate: Date?
    let filePath: String
...
struct TaskMapping: Codable {
    let obsidianId: String
    let reminderId: String
    let filePath: String
    let taskText: String  // Store the actual task text for better matching
    ...
    func findMappingByReminderId(_ reminderId: String) -> TaskMapping? {
        // Find mapping by reminder ID
        return TaskMappingStore(mappings: [self]).findMapping(reminderId: reminderId)
    }
}

struct TaskMappingStore: Codable {
    var mappings: [TaskMapping]
    
    // Find mapping by both file path and task text
    func findMapping(filePath: String, taskText: String) -> TaskMapping? {
    ...
    func findMapping(reminderId: String) -> TaskMapping? {
        return mappings.first { $0.reminderId == reminderId }
    }
}

func loadTaskMappings(vaultPath: String) throws -> TaskMappingStore {
...
func saveTaskMappings(_ store: TaskMappingStore, vaultPath: String) throws {
    let mappingFile = (vaultPath as NSString).appendingPathComponent("._RemindersMapping.json")
    do {
...
func findIncompleteTasks(in vaultPath: String) throws -> [ObsidianTask] {
    ...
    var updatedFiles: [(URL, String)] = []  // Store file updates for later
...
func findCompletedTasks(in vaultPath: String) throws -> [ObsidianTask] {
    var tasks: [ObsidianTask] = []
    let fileManager = FileManager.default
    let enumerator = fileManager.enumerator(
        at: URL(fileURLWithPath: vaultPath),
...
func cleanupTaskIds(in vaultPath: String) throws {
    let fileManager = FileManager.default
    let enumerator = fileManager.enumerator(
        at: URL(fileURLWithPath: vaultPath),
        includingPropertiesForKeys: [.isRegularFileKey],
...
struct RemindersSyncCLI {
    static func main() async {
        ...
        do {
            ...
            // If mapping file doesn't exist, clean up any existing task IDs first
